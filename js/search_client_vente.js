class ClientSearch {
  ...
  async searchClients(searchText) {
      try {
          ...
          const response = await fetch("/Pharmacie_S/PHP/clients/search_clients.php", {
              method: "POST",
              headers: {
  ...
  initializeEventListeners() {
      ...
      // Gestionnaire pour le select
      this.clientSelect.addEventListener("change", () => {
          // Toujours vider la barre de recherche quand on change la sélection dans le select
          this.clearSearch();
      });
  }
}
